...
int DijkstraMagic(problema prob, int **wt, vertex **st,int Xa, int Ya, int Xb, int Yb, solucao *sol, int *idx){
  Heap *heap = NULL;
  HeapNode *V; 
...
solucao *modoB (problema prob, vertex **st, int **wt){
  ...
  {
    stop = DijkstraMagic(prob, wt, st, prob.pontos[i].x, prob.pontos[i].y, prob.pontos[i+1].x, prob.pontos[i+1].y, sol, &idx);
    if (stop) break;
  }
  return sol;
}
...
solucao *modoA (problema prob, vertex **st, int **wt){
  ...
  int idx = 0;
  InitSolution(&sol, (prob.npontos)-1);
  DijkstraMagic(prob, wt, st, prob.pontos[0].x, prob.pontos[0].y, prob.pontos[1].x, prob.pontos[1].y, sol, &idx);
  return sol;
}
